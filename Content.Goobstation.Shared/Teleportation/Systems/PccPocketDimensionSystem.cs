...
public sealed partial class PccPocketDimensionSystem : EntitySystem
{
    ...
    [SubscribeLocalEvent]
    private void OnShutdown(Entity<PccPocketDimensionComponent> ent, ref ComponentShutdown args)
    {
        if (ent.Comp.PccPocketDimensionMap is { } map && Exists(map))
            PredictedQueueDel(map);
    }

    [SubscribeLocalEvent]
    private void OnGetVerbs(Entity<PccPocketDimensionComponent> ent, ref GetVerbsEvent<AlternativeVerb> args)
    {
    ...
    /// </summary>
    private void HandleActivation(Entity<PccPocketDimensionComponent> ent, EntityUid user)
    {
        var (uid, comp) = ent;
        if (Deleted(comp.PccPocketDimensionMap))
        {
            if (!_timing.IsFirstTimePredicted)
                return; // dont want to try loading a map 10 times lol

            if (!_mapLoader.TryLoadMap(comp.PccPocketDimensionPath, out var map, out var roots,
                options: new Robust.Shared.EntitySerialization.DeserializationOptions { InitializeMaps = true }))
            {
                Log.Error($"Failed to load pocket dimension map {comp.PccPocketDimensionPath}");
                QueueDel(map);
                return;
            }

            comp.PccPocketDimensionMap = map;
            ...
            if (!foundGrid)
            {
                Log.Error($"Pocket dimension {comp.PccPocketDimensionPath} had no grids!");
                Del(map);
                comp.PccPocketDimensionMap = null;
                return;
            }
            Dirty(ent);
        }
    ...
    private void SpawnExitPortal(Entity<PccPocketDimensionComponent> ent, EntityUid grid)
    {
