    ...
    public async Task NoStaticPriceAndStackPrice()
    {
        await Server.WaitAssertion(() =>
        {
            using (Assert.EnterMultipleScope())
            {
                ...
                // <Trauma>
                var stackName = _sCompFact.CompName<StackComponent>();
                var stackPriceName = _sCompFact.CompName<StackPriceComponent>();
                // </Trauma>
                foreach (var (proto, staticPriceComp) in protoIds)
                {
                    if (
                        proto.TryComp<StackPriceComponent>(stackPriceName, out var stackPriceComp) // Trauma - use TryComp with stackPriceName from above
                    ...
                    if (proto.HasComp(stackName)) // Trauma - use stackName from above
                    {
                        Assert.That(
                            staticPriceComp.Price,
                            Is.EqualTo(0),
